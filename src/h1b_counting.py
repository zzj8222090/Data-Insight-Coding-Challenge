...
def top_certified(name):
    ...
    occupation = [record.split(';')[occupation_col] for record in lines[1:] if record.split(';')[status_col]=='CERTIFIED' and record.split(';')[occupation_col]!='']
    states = [record.split(';')[state_col] for record in lines[1:] if record.split(';')[status_col]=='CERTIFIED' and record.split(';')[state_col]!='']
